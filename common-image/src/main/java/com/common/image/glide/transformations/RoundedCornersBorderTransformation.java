package com.common.image.glide.transformations;
...
public class RoundedCornersBorderTransformation implements Transformation<Bitmap> {
    ...
    public RoundedCornersBorderTransformation(Context context, int radius, int margin, int width, int color) {
        this(context, radius, margin, CornerType.ALL, width, color);
    }

    public RoundedCornersBorderTransformation(BitmapPool pool, int radius, int margin, int width, int color) {
        this(pool, radius, margin, CornerType.ALL, width, color);
    }

    public RoundedCornersBorderTransformation(Context context, int radius, int margin, CornerType cornerType,
        int width, int color) {
        this(Glide.get(context).getBitmapPool(), radius, margin, cornerType, width, color);
    }

    public RoundedCornersBorderTransformation(BitmapPool pool, int radius, int margin, CornerType cornerType,
        int width, int color) {
        mBitmapPool = pool;
        mRadius = radius;
        mDiameter = mRadius * 2;
    ...
    public Resource<Bitmap> transform(@NonNull Context context, @NonNull Resource<Bitmap> resource, int outWidth, int outHeight) {
        ...
        if (mBorderWidth > 0) {
            ...
            canvas.drawRoundRect(new RectF(mMargin, mMargin, width, height), mRadius, mRadius, borderPaint);
            mMargin += mBorderWidth;
    ...
    private void drawTopRightRoundRect(Canvas canvas, Paint paint, float right, float bottom) {
        canvas
            .drawRoundRect(new RectF(right - mDiameter, mMargin, right, mMargin + mDiameter), mRadius, mRadius, paint);
    ...
    public void updateDiskCacheKey(@NonNull MessageDigest messageDigest) {
        String key = "RoundedCornersBorderTransformation(radius=" + mRadius + ", margin=" + mMargin + ", diameter="
